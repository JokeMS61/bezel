...
def getHour(localtime):
   return time.strftime('%H', localtime)

def getMinute(localtime):
   return time.strftime('%M', localtime)

def getSecond(localtime):
   return time.strftime('%S', localtime)
...
if __name__ == "__main__": #if running this module as a stand-alone program
    ...
    for x in range(0, 100):
        ...
        print("start: " + str(tStart) + " , " + "Now: " + str(tNow) + " , diff: " + str(dt))
        tStart = tNow #us; update
    end = micros()
    print ("excution time: " + str((end-start) / 1000000) +  " seconds")
